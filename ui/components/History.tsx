  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      {runs.length > 3 && (
        <div className="px-3 pt-3">
          <input
            ...
            placeholder={`Search ${runs.length} runs: question, answer, kind`}
      ...
      <div className="scroll-thin min-h-0 flex-1 overflow-y-auto px-2 py-2">
        {shown.length === 0 && <div className="px-2 py-3 text-center text-xs text-stone-500">No run matches “{needle}”</div>}
        <ul ref={list}>
          {shown.map((r) => {
            ...
            return (
              <li key={r.id} className={cx("group relative rounded-lg", r.id === current ? "bg-white shadow-sm ring-1 ring-stone-200" : "hover:bg-stone-100")}>
                <button data-run={r.id} onClick={() => onOpen(r)} aria-current={r.id === current ? "true" : undefined} className="w-full rounded-lg px-2.5 py-2 pr-12 text-left">
                  ...
                  {(answer || kind) && (
                    <div className="mt-0.5 flex min-w-0 items-baseline gap-1.5" title={answer}>
                      {kind && <span className="shrink-0 rounded bg-teal-50 px-1 text-[10px] font-semibold text-teal-800">{kind}</span>}
                      {answer && (
                        <span className={cx("truncate font-serif text-[13px] font-semibold", r.end?.report?.status === "below" ? "text-amber-800" : "text-stone-900")}>
                  ...
                  <div className="mt-1 flex items-center gap-1.5 text-[11px] whitespace-nowrap text-stone-500" title={`${o.label} · ${TOOL_LABEL[r.request.tool]}${kind ? ` · ${kind}` : ""} · ${new Date(r.at).toLocaleString()}`}>
                    <span className={cx("h-1.5 w-1.5 shrink-0 rounded-full", o.dot)} />
                    <span className="truncate">{o.label}</span>
                    <span className="rounded bg-stone-200/60 px-1 text-stone-600">{TOOL_LABEL[r.request.tool]}</span>
                    <span className="ml-auto tabular-nums">{dollars(runSpend(r))}</span>
                    <span>{ago(r.at, now)}</span>
                  </div>
                </button>
                <div className={cx("absolute top-1.5 right-1.5 flex gap-0.5", !r.pinned && "fine:opacity-0 group-hover:opacity-100 focus-within:opacity-100")}>
                  <button
                    onClick={() => onPin(r.id)}
                    ...
                    className={cx("rounded p-1", r.pinned ? "text-teal-700" : "text-stone-500 hover:text-stone-700")}
                  >
                    <Icon name="star" size={13} filled={r.pinned} />
                  </button>
                  <button onClick={() => remove(r.id)} aria-label="Delete" title="Delete this run" className="rounded p-1 text-stone-500 hover:text-rose-600">
