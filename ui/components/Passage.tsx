...
export function Rows({ rows, first }: { rows: Row[]; first?: boolean }) {
  ...
  return (
    <div className="scroll-thin overflow-x-auto rounded-lg ring-1 ring-stone-200">
...
export function Passage({ paras }: { paras: Para[] }) {
  ...
  for (let i = 0; i < paras.length; i++) {
    ...
    if (p.table) {
      ...
      blocks.push(<Rows key={i} rows={rows} />);
    } else if (p.heading)
      blocks.push(
