...
/**
 * The columns of -layout text, each read down in turn. A gutter is a
 * stretch of spaces most lines share at the same place; a line that runs
 * across one, a heading or a table row, ends every column and stands on
 * its own. Neither extractor read these books in order on its own:
 * pdftotext interleaved the Fallout and Heart columns line by line, its
 * -raw order glued words on Legend in the Mist, and mutool glued words on
 * Heart. The gutters are found from the column layout they all keep, three
 * of them on Heart's equipment tags page.
 */
export function columns(text: string): string {
  ...
  // a gutter is a stretch that most of them share, away from the margins.
  ...
  for (let x = Math.floor(width * 0.15); x < width * 0.85; x++) {
    if (band[x]! < wide.length * 0.6) continue;
    const last = gutters.at(-1);
    if (last && last.end === x) last.end = x + 1;
    else gutters.push({ start: x, end: x + 1 });
  }
  if (gutters.length === 0) return lines.join("\n");
  const out: string[] = [];
  let cols: string[][] = gutters.map(() => []).concat([[]]);
  const flush = () => {
    for (const c of cols) if (c.length) out.push(...c, "");
    cols = gutters.map(() => []).concat([[]]);
  };
  for (const l of lines) {
    // Cut the line at each gutter it reaches; its own gap may sit a little
    // off the gutter when the text before it runs long.
    // Three spaces found the gutter, and such a run may sit a little off it
    // when the text before it runs long; two spaces suffice only inside it.
    // One run of spaces may cover two gutters when the column between is empty.
    const gaps = [...l.matchAll(/ {2,}/g)].map((m) => ({ start: m.index!, end: m.index! + m[0].length, slack: m[0].length >= 3 ? 3 : 0 }));
    const cells: string[] = [];
    let at = 0;
    let spans = false;
    for (const g of gutters) {
      ...
      cells.push(l.slice(at, Math.max(at, gap.start)).trim());
    ...
    cells.push(l.slice(at).trim());
    cells.forEach((c, i) => c && cols[i]!.push(c));
  }
  flush();
  return out.join("\n").replace(/\n{3,}/g, "\n\n");
}
