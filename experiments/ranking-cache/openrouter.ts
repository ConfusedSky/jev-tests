...
const models = ["openai/text-embedding-3-small", "openai/text-embedding-3-large", "qwen/qwen3-embedding-4b", "qwen/qwen3-embedding-8b", "google/gemini-embedding-001", "baai/bge-large-en-v1.5"];
...
for (const model of models) {
  try {
    ...
    let cost = 0, tok = 0;
    for (const k of ["q", "qi", "rich"] as const) { const e = await embed(model, texts[k]); out[k] = e.vecs; cost += e.usage?.cost ?? 0; tok += e.usage?.prompt_tokens ?? 0; }
    ...
    lat.sort((a, b) => a - b);
    Object.assign(out, { costPerQuery: cost / texts.q.length / 3, tokensPerQuery: tok / texts.q.length / 3, msPerQuery: lat[2], dim: (out.q as number[][])[0]!.length });
    await Bun.write(`${import.meta.dir}/embs/or_${model.split("/")[1]}.json`, JSON.stringify(out));
