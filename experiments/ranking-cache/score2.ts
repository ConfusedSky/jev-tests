...
for (const f of [...new Glob("embs2/*.json").scanSync(import.meta.dir)].sort()) {
  const e = await Bun.file(`${import.meta.dir}/${f}`).json();
  const id = f.slice(6, -5);
  const variants: [string, string, string][] = [["q~rich", "q", "rich"], ["qi~rich", "qi", "rich"], ["s~s", "s", "s"], ["si~s", "si", "s"], ["s~srich", "s", "srich"], ["si~srich", "si", "srich"], ["sq~sq", "sq", "sq"], ["sqi~sq", "sqi", "sq"], ["sq~sqrich", "sq", "sqrich"], ["sqi~sqrich", "sqi", "sqrich"]];
...
const probe: [number, number][] = [[11, 9], [9, 11], [3, 9], [9, 10], [10, 9], [12, 13], [13, 11], [11, 13], [7, 6], [6, 7], [8, 6], [8, 7], [14, 15], [15, 8], [2, 3], [3, 5], [0, 1], [2, 16], [24, 23], [25, 24], [24, 25], [22, 21], [22, 26], [26, 27], [31, 32], [33, 34], [39, 17]];
const pick = (process.env.PROBE ?? "").split(",").filter(Boolean);
if (pick.length) {
  const ms = pick.map((n) => rows.find((r) => r.m.name === n)!);
  console.log("\npair (new <- cached) | good | " + pick.map((n) => `${n} [t=${ms[pick.indexOf(n)]!.w.p90?.t.toFixed(3)}]`).join(" | "));
  for (const [a, b] of probe) console.log(`${Q[a]!.q.slice(0, 42)} <- ${Q[b]!.q.slice(0, 42)} | ${good(a, b) ? "good" : "bad"} | ` + ms.map((r) => { const v = r.m.sim(a, b); return `${v.toFixed(3)}${v >= (r.w.p90?.t ?? 9) ? " YES" : ""}`; }).join(" | "));
  for (const r of ms) { const t = r.w.p90!.t; console.log(`\n${r.m.name} at P>=90% t=${t.toFixed(3)} wrong:`); for (const [a, b] of pairs) if (r.m.sim(a, b) >= t && !good(a, b)) console.log(`  ${r.m.sim(a, b).toFixed(3)} ${Q[a]!.q} <- ${Q[b]!.q}`); }
}
