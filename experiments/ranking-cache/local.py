# Embeds texts.json with local models; writes embs/local_<name>.json. Usage: local.py <hf id> [st|siglip] [cuda|cpu]
import json, sys, time, statistics, torch
S = __file__.rsplit("/", 1)[0]
T = json.load(open(f"{S}/texts.json"))
...
load = time.time() - t0
out = {k: enc(T[k]) for k in ["q", "qi", "rich"]}
lat = []
for q in T["q"][:7]:
...
json.dump(out, open(f"{S}/embs/local_{mid.split('/')[-1]}.json", "w"))
